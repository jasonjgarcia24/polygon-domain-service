...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata _name) public payable {
        ...
        if (msg.value < price(_name)) revert InsufficientFunds();

        domains[_name] = msg.sender;
        console.log("%s has registered a domain!", msg.sender);

        // Combine the name passed into the function with the TLD
        string memory _fullName = string(abi.encodePacked(_name, ".", tld));

        // Create the SVG (image) for the NFT with the name
        string memory _finalSvg = string(
            abi.encodePacked(svgPartOne, _fullName, svgPartTwo)
        );

        uint256 _newRecord = tokenIds.current();
        uint256 _len = StringUtils.strlen(_name);
        string memory _strLen = Strings.toString(_len);
        ...
        string memory _finalTokenUri = string(
        ...
        console.log("Final tokenURI", _finalTokenUri);
        console.log(
            "--------------------------------------------------------\n"
        );

        _safeMint(msg.sender, _newRecord);
        _setTokenURI(_newRecord, _finalTokenUri);

        domains[_name] = msg.sender;
        names[_newRecord] = _name;
        tokenIds.increment();
    }
    ...
    function valid(string calldata _name) public pure returns (bool) {
        return
            StringUtils.strlen(_name) >= 3 && StringUtils.strlen(_name) <= 10;
    }
